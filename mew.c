...
void
server_terminate(struct mew_server *server)
{
...
{
	struct mew_server *server = wl_container_of(listener, server, display_destroy);
...
{
	struct mew_server *server = data;

	/* Close Mew's read pipe. */
	close(fd);
...
static bool
spawn_primary_client(struct mew_server *server, char *argv[], pid_t *pid_out, struct wl_event_source **sigchld_source)
{
	int fd[2];
	if (pipe(fd) != 0) {
	...
	*pid_out = pid;
	...
	/* Close write, we only need read in Mew. */
	close(fd[1]);
...
static void
usage(FILE *file, const char *mew)
{
	fprintf(file,
...
static bool
parse_args(struct mew_server *server, int argc, char *argv[])
{
	int c;
...
{
	struct mew_server server = {.log_level = WLR_INFO};
	...
	if (setenv("WAYLAND_DISPLAY", socket, true) < 0) {
		...
		wlr_log(WLR_DEBUG, "Mew is running on Wayland display %s", socket);
	}
