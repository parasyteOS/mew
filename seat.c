...
static void drag_icon_update_position(struct mew_drag_icon *drag_icon);
 ...
 * nothing.
 */
static struct mew_view *
desktop_view_at(struct mew_server *server, double lx, double ly, struct wlr_surface **surface, double *sx, double *sy)
{
...
static void
press_cursor_button(struct mew_seat *seat, struct wlr_input_device *device, uint32_t time, uint32_t button,
		    uint32_t state, double lx, double ly)
{
	struct mew_server *server = seat->server;

	if (state == WLR_BUTTON_PRESSED) {
		double sx, sy;
		struct wlr_surface *surface;
		struct mew_view *view = desktop_view_at(server, lx, ly, &surface, &sx, &sy);
		struct mew_view *current = seat_get_focus(seat);
		if (view == current) {
			return;
		}
...
static void
update_capabilities(struct mew_seat *seat)
{
	uint32_t caps = 0;
...
static void
map_input_device_to_output(struct mew_seat *seat, struct wlr_input_device *device, const char *output_name)
{
	if (!output_name) {
...
{
	struct mew_touch *touch = wl_container_of(listener, touch, destroy);
	struct mew_seat *seat = touch->seat;
...
static void
handle_new_touch(struct mew_seat *seat, struct wlr_touch *wlr_touch)
{
	struct mew_touch *touch = calloc(1, sizeof(struct mew_touch));
	if (!touch) {
...
{
	struct mew_pointer *pointer = wl_container_of(listener, pointer, destroy);
	struct mew_seat *seat = pointer->seat;
...
static void
handle_new_pointer(struct mew_seat *seat, struct wlr_pointer *wlr_pointer)
{
	struct mew_pointer *pointer = calloc(1, sizeof(struct mew_pointer));
	if (!pointer) {
...
{
	struct mew_server *server = wl_container_of(listener, server, new_virtual_pointer);
	struct mew_seat *seat = server->seat;
...
static void
handle_modifier_event(struct wlr_keyboard *keyboard, struct mew_seat *seat)
{
...
static bool
handle_keybinding(struct mew_server *server, xkb_keysym_t sym)
{
#ifdef DEBUG
...
static void
handle_key_event(struct wlr_keyboard *keyboard, struct mew_seat *seat, void *data)
{
...
{
	struct mew_keyboard_group *mew_group = wl_container_of(listener, mew_group, key);
	handle_key_event(&mew_group->wlr_group->keyboard, mew_group->seat, data);
}

static void
handle_keyboard_group_modifiers(struct wl_listener *listener, void *data)
{
	struct mew_keyboard_group *group = wl_container_of(listener, group, modifiers);
	handle_modifier_event(&group->wlr_group->keyboard, group->seat);
}

static void
mew_keyboard_group_add(struct wlr_keyboard *keyboard, struct mew_seat *seat, bool virtual)
{
	/* We apparently should not group virtual keyboards,
	 * so create a new group with it
	 */
	if (!virtual) {
		struct mew_keyboard_group *group;
	 ...
	 * any group */
	struct mew_keyboard_group *mew_group = calloc(1, sizeof(struct mew_keyboard_group));
	if (mew_group == NULL) {
		wlr_log(WLR_ERROR, "Failed to allocate keyboard group.");
		return;
	}
	mew_group->seat = seat;
	mew_group->is_virtual = virtual;
	mew_group->wlr_group = wlr_keyboard_group_create();
	if (mew_group->wlr_group == NULL) {
		wlr_log(WLR_ERROR, "Failed to create wlr keyboard group.");
		goto cleanup;
	}

	mew_group->wlr_group->data = mew_group;
	wlr_keyboard_set_keymap(&mew_group->wlr_group->keyboard, keyboard->keymap);

	wlr_keyboard_set_repeat_info(&mew_group->wlr_group->keyboard, keyboard->repeat_info.rate,
				     keyboard->repeat_info.delay);

	wlr_log(WLR_DEBUG, "Created keyboard group");

	wlr_keyboard_group_add_keyboard(mew_group->wlr_group, keyboard);
	wl_list_insert(&seat->keyboard_groups, &mew_group->link);

	wl_signal_add(&mew_group->wlr_group->keyboard.events.key, &mew_group->key);
	mew_group->key.notify = handle_keyboard_group_key;
	wl_signal_add(&mew_group->wlr_group->keyboard.events.modifiers, &mew_group->modifiers);
	mew_group->modifiers.notify = handle_keyboard_group_modifiers;

	return;

cleanup:
	if (mew_group && mew_group->wlr_group) {
		wlr_keyboard_group_destroy(mew_group->wlr_group);
	}
	free(mew_group);
}

static void
keyboard_group_destroy(struct mew_keyboard_group *keyboard_group)
{
...
static void
handle_new_keyboard(struct mew_seat *seat, struct wlr_keyboard *keyboard, bool virtual)
{
	...
	mew_keyboard_group_add(keyboard, seat, virtual);
...
{
	struct mew_server *server = wl_container_of(listener, server, new_virtual_keyboard);
	struct mew_seat *seat = server->seat;
...
{
	struct mew_seat *seat = wl_container_of(listener, seat, new_input);
...
{
	struct mew_seat *seat = wl_container_of(listener, seat, request_set_primary_selection);
...
{
	struct mew_seat *seat = wl_container_of(listener, seat, request_set_selection);
...
{
	struct mew_seat *seat = wl_container_of(listener, seat, request_set_cursor);
...
{
	struct mew_seat *seat = wl_container_of(listener, seat, touch_down);
	...
	double sx, sy;
	struct wlr_surface *surface;
	struct mew_view *view = desktop_view_at(seat->server, lx, ly, &surface, &sx, &sy);
...
{
	struct mew_seat *seat = wl_container_of(listener, seat, touch_up);
...
{
	struct mew_seat *seat = wl_container_of(listener, seat, touch_motion);
...
{
	struct mew_seat *seat = wl_container_of(listener, seat, touch_frame);
...
{
	struct mew_seat *seat = wl_container_of(listener, seat, cursor_frame);
...
{
	struct mew_seat *seat = wl_container_of(listener, seat, cursor_axis);
...
{
	struct mew_seat *seat = wl_container_of(listener, seat, cursor_button);
...
static void
process_cursor_motion(struct mew_seat *seat, uint32_t time_msec, double dx, double dy, double dx_unaccel,
		      double dy_unaccel)
{
	double sx, sy;
	...
	struct mew_view *view = desktop_view_at(seat->server, seat->cursor->x, seat->cursor->y, &surface, &sx, &sy);
	if (!view) {
	...
	struct mew_drag_icon *drag_icon;
...
{
	struct mew_seat *seat = wl_container_of(listener, seat, cursor_motion_absolute);
...
{
	struct mew_seat *seat = wl_container_of(listener, seat, cursor_motion_relative);
...
static void
drag_icon_update_position(struct mew_drag_icon *drag_icon)
{
	struct wlr_drag_icon *wlr_icon = drag_icon->wlr_drag_icon;
	struct mew_seat *seat = drag_icon->seat;
...
{
	struct mew_drag_icon *drag_icon = wl_container_of(listener, drag_icon, destroy);
...
{
	struct mew_seat *seat = wl_container_of(listener, seat, request_start_drag);
...
{
	struct mew_seat *seat = wl_container_of(listener, seat, start_drag);
	...
	struct mew_drag_icon *drag_icon = calloc(1, sizeof(struct mew_drag_icon));
	if (!drag_icon) {
		return;
	}
	drag_icon->seat = seat;
...
{
	struct mew_seat *seat = wl_container_of(listener, seat, destroy);
...
struct mew_seat *
seat_create(struct mew_server *server, struct wlr_backend *backend)
{
	struct mew_seat *seat = calloc(1, sizeof(struct mew_seat));
	if (!seat) {
...
void
seat_destroy(struct mew_seat *seat)
{
	if (!seat) {
		return;
	}
	...
	struct mew_keyboard_group *keyboard_group, *keyboard_group_tmp;
...
struct mew_view *
seat_get_focus(struct mew_seat *seat)
{
...
void
seat_set_focus(struct mew_seat *seat, struct mew_view *view)
{
	struct mew_server *server = seat->server;
	struct wlr_seat *wlr_seat = seat->seat;
	struct mew_view *prev_view = seat_get_focus(seat);
...
void
seat_center_cursor(struct mew_seat *seat)
{
