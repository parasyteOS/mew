...
static void
xdg_decoration_set_mode(struct mew_xdg_decoration *xdg_decoration)
{
	wlr_xdg_toplevel_decoration_v1_set_mode(xdg_decoration->wlr_decoration, WLR_XDG_TOPLEVEL_DECORATION_V1_MODE_SERVER_SIDE);
}

static void
xdg_decoration_handle_destroy(struct wl_listener *listener, void *data)
{
	struct mew_xdg_decoration *xdg_decoration = wl_container_of(listener, xdg_decoration, destroy);
...
{
	struct mew_xdg_decoration *xdg_decoration = wl_container_of(listener, xdg_decoration, commit);
...
{
	struct mew_xdg_decoration *xdg_decoration = wl_container_of(listener, xdg_decoration, request_mode);
...
{
	struct mew_view *view = popup_get_view(popup);
	if (view == NULL) {
		return;
	}
...
static struct mew_xdg_shell_view *
xdg_shell_view_from_view(struct mew_view *view)
{
	return (struct mew_xdg_shell_view *) view;
}

static char *
get_title(struct mew_view *view)
{
	struct mew_xdg_shell_view *xdg_shell_view = xdg_shell_view_from_view(view);
	return xdg_shell_view->xdg_toplevel->title;
}

static void
get_geometry(struct mew_view *view, int *width_out, int *height_out)
{
	struct mew_xdg_shell_view *xdg_shell_view = xdg_shell_view_from_view(view);
	struct wlr_box geom;
...
static bool
is_primary(struct mew_view *view)
{
	struct mew_xdg_shell_view *xdg_shell_view = xdg_shell_view_from_view(view);
...
static bool
is_transient_for(struct mew_view *child, struct mew_view *parent)
{
	struct mew_xdg_shell_view *_child = xdg_shell_view_from_view(child);
	struct wlr_xdg_toplevel *xdg_toplevel = _child->xdg_toplevel;
	struct mew_xdg_shell_view *_parent = xdg_shell_view_from_view(parent);
	while (xdg_toplevel) {
...
static void
activate(struct mew_view *view, bool activate)
{
	struct mew_xdg_shell_view *xdg_shell_view = xdg_shell_view_from_view(view);
...
static void
maximize(struct mew_view *view, int output_width, int output_height)
{
	struct mew_xdg_shell_view *xdg_shell_view = xdg_shell_view_from_view(view);
...
{
	struct mew_xdg_shell_view *xdg_shell_view = wl_container_of(listener, xdg_shell_view, request_fullscreen);
...
{
	struct mew_xdg_shell_view *xdg_shell_view = wl_container_of(listener, xdg_shell_view, unmap);
	struct mew_view *view = &xdg_shell_view->view;
...
{
	struct mew_xdg_shell_view *xdg_shell_view = wl_container_of(listener, xdg_shell_view, map);
	struct mew_view *view = &xdg_shell_view->view;
...
{
	struct mew_xdg_shell_view *xdg_shell_view = wl_container_of(listener, xdg_shell_view, commit);
...
{
	struct mew_xdg_shell_view *xdg_shell_view = wl_container_of(listener, xdg_shell_view, destroy);
	struct mew_view *view = &xdg_shell_view->view;
...
static const struct mew_view_impl xdg_shell_view_impl = {
	.get_title = get_title,
...
{
	struct mew_server *server = wl_container_of(listener, server, new_xdg_toplevel);
	struct wlr_xdg_toplevel *toplevel = data;

	struct mew_xdg_shell_view *xdg_shell_view = calloc(1, sizeof(struct mew_xdg_shell_view));
	if (!xdg_shell_view) {
		wlr_log(WLR_ERROR, "Failed to allocate XDG Shell view");
		return;
	}

	view_init(&xdg_shell_view->view, server, &xdg_shell_view_impl);
...
{
	struct mew_xdg_popup *popup = wl_container_of(listener, popup, destroy);
...
{
	struct mew_xdg_popup *popup = wl_container_of(listener, popup, commit);
...
{
	struct mew_server *server = wl_container_of(listener, server, new_xdg_popup);
	struct wlr_xdg_popup *wlr_popup = data;

	struct mew_view *view = popup_get_view(wlr_popup);
	if (view == NULL) {
		return;
	}
	...
	struct mew_xdg_popup *popup = calloc(1, sizeof(*popup));
	if (popup == NULL) {
...
{
	struct mew_server *server = wl_container_of(listener, server, xdg_toplevel_decoration);
	struct wlr_xdg_toplevel_decoration_v1 *wlr_decoration = data;

	struct mew_xdg_decoration *xdg_decoration = calloc(1, sizeof(struct mew_xdg_decoration));
	if (!xdg_decoration) {
		return;
	}
